"""
Build continuous (multi-second) 32-channel NMED dataset from existing 1-second processed data.

Groups consecutive 1-second EEG windows + EnCodec embeddings into multi-second sequences.
Uses the 32-channel (downsampled from 120ch EGI) format for compatibility with SongFam data.

Usage:
    uv run python -m data.nmed.build_continuous_32ch --window-seconds 2
    uv run python -m data.nmed.build_continuous_32ch --window-seconds 5
    uv run python -m data.nmed.build_continuous_32ch --window-seconds 10
    uv run python -m data.nmed.build_continuous_32ch --window-seconds 30
    uv run python -m data.nmed.build_continuous_32ch --window-seconds -1   # full song
"""
...
CONTINUOUS_DIR = NMED_OUTPUT_DIR / "continuous-32ch"


def build_continuous(window_seconds: int):
    """Group existing 1-second 32ch windows into multi-second sequences."""
    full_song = window_seconds == -1
    label = "full" if full_song else f"w{window_seconds}"
    out_dir = CONTINUOUS_DIR / label
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"=== Building 32ch continuous data: {label} ===")

    for split in ["train", "val"]:
        print(f"\n[{split}]")

        # Load existing 1-second 32ch data
        eeg_path = NMED_OUTPUT_DIR / f"nmed-32ch-{split}.safetensors"
        encodec_path = NMED_OUTPUT_DIR / f"nmed-32ch-{split}-encodec.safetensors"
        meta_path = NMED_OUTPUT_DIR / f"nmed-32ch-{split}-metadata.parquet"

        print(f"  Loading from {eeg_path}...")
        eeg_data = load_file(eeg_path)
        eeg_samples = eeg_data["samples"]  # (N, 32, 125)
        ...
        for row in tqdm(
            ...
            if full_song:
                seq_eeg = eeg_samples[idx_list]
                seq_audio = audio_embeds[idx_list]
                ...
                for seq_idx in range(n_sequences):
                    ...
                    seq_eeg = eeg_samples[chunk_indices]
                    seq_audio = audio_embeds[chunk_indices]
                    sequences_eeg.append(seq_eeg)
                    sequences_audio.append(seq_audio)
                    sequences_meta.append({
                        "subject_id": subj,
        ...
        if full_song:
            ...
            stacked_eeg = torch.stack(sequences_eeg)
            stacked_audio = torch.stack(sequences_audio)

            save_file(
                {"sparse_samples": stacked_eeg},
                out_dir / f"nmed-{split}.safetensors",
            )
            save_file(
...
def main():
    parser = argparse.ArgumentParser(description="Build continuous 32ch NMED data")
    parser.add_argument(
        "--window-seconds",
        type=int,
        required=True,
