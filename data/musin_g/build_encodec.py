"""
Pre-compute EnCodec encoder embeddings for all MUSIN-G audio stimuli.

Produces audio_embeds tensors aligned with the EEG sample ordering
from the processed dataset.

Usage:
    uv run python -m data.musin_g.build_encodec
"""
...
from data.musin_g.songs import MUSIN_G_AUDIO_DIR, MUSIN_G_OUTPUT_DIR, SONGS
...
def load_audio_24khz(wav_path: Path) -> torch.Tensor:
...
def build():
    print("=== MUSIN-G EnCodec Embedding Builder ===")
    ...
    for song in tqdm(SONGS, desc="Encoding"):
        wav_path = MUSIN_G_AUDIO_DIR / song.audio_filename
        if not wav_path.exists():
            print(f"  WARNING: Audio not found: {wav_path}")
            continue

        audio = load_audio_24khz(wav_path)

        if torch.cuda.is_available():
            audio = audio.cuda()

        windows = encode_audio(model, audio)
        song_embeds[song.id] = windows.cpu()
        print(f"  Song {song.id} ({song.title}): {windows.shape[0]} windows")

    # Build per-split embedding tensors aligned with EEG metadata ordering
    print("\n[3/3] Building aligned tensors...")

    for split in ["train", "val"]:
        meta_path = MUSIN_G_OUTPUT_DIR / f"musin-g-{split}-metadata.parquet"
        ...
        for i in range(n_samples):
            ...
            if song_id not in song_embeds:
                skipped += 1
                continue

            embeds = song_embeds[song_id]
        ...
        out_path = MUSIN_G_OUTPUT_DIR / f"musin-g-{split}-encodec.safetensors"
