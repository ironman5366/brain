"""
Pre-compute EnCodec encoder embeddings for all DS005876 audio stimuli.

Produces audio_embeds tensors aligned with the EEG sample ordering
from the processed dataset.

Usage:
    uv run python -m data.songfam.build_encodec
"""
...
from data.songfam.songs import SONGFAM_OUTPUT_DIR, SONGFAM_STIMULI_DIR
...
def build():
    print("=== DS005876 EnCodec Embedding Builder ===")
    ...
    wav_files = sorted(SONGFAM_STIMULI_DIR.glob("*.wav"))
    ...
    for split in ["train", "val"]:
        ...
        for i in range(n_samples):
            ...
            if song_filename not in song_embeds:
                skipped += 1
                continue

            embeds = song_embeds[song_filename]
        ...
        out_path = SONGFAM_OUTPUT_DIR / f"songfam-{split}-encodec.safetensors"
