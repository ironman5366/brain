...
def main():
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        help="Path to checkpoint (default: checkpoints/<config_name>/final)",
    )
    ...
    with torch.inference_mode():
        ...
        model = EEGNet.from_pretrained(checkpoint_path).to(DEVICE)
        model.eval()

        if dataset_type == "sparse_classification":
        ...
        print(f"\n--- Per-Class Metrics ---")
        print(
            f"  {'Class':<8} {'Target':>8} {'Pred':>8} {'Prec':>8} {'Recall':>8} {'F1':>8}"
        )
